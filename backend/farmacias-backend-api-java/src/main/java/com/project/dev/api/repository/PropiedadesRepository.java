/*
 * @fileoverview    {PropiedadesRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code PropiedadesRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface PropiedadesRepository extends JpaRepository<Propiedades, Long> {
     ...
     */
    public List<Propiedades> findByIntIdPropiedad(Long id);
    ...
    @Query("SELECT m FROM Propiedades m WHERE m.intIdPropiedad LIKE CONCAT('%', :strSearch, '%')")
    public List<Propiedades> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Propiedades m WHERE m.intIdPropiedad LIKE CONCAT('%', :strSearch, '%')")
    public Page<Propiedades> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
