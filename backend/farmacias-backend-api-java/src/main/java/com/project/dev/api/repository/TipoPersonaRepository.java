/*
 * @fileoverview    {TipoPersonaRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code TipoPersonaRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface TipoPersonaRepository extends JpaRepository<TipoPersona, Long> {
     ...
     */
    public List<TipoPersona> findByIntIdTipoPersona(Long id);
    ...
    @Query("SELECT m FROM TipoPersona m WHERE m.intIdTipoPersona LIKE CONCAT('%', :strSearch, '%')")
    public List<TipoPersona> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM TipoPersona m WHERE m.intIdTipoPersona LIKE CONCAT('%', :strSearch, '%')")
    public Page<TipoPersona> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
